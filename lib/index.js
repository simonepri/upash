...
function toHash(password, options, callback) {
  ...
  hashFunc.generate(password, options.configs, (err, hash, data) => {
    if (err) {
      return callback(err);
    }
 ...
 */
  ...
  * @param  {object} err Possible error thrown.
  * @param  {boolean} valid True if the hash computed for the input matches.
  */
function verifyHash(hash, input, callback) {
  ...
  hashFunc.hash(input, hashObj.data, (err, inputHash) => {
    if (err) {
      return callback(err);
    }

    callback(null, tsse(inputHash, hashObj.hash));
  });
}
